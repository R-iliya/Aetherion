...
namespace Aetherion
{
	...
	struct PhysicsGeometryManager final : ResourceManager
	{
		...
		Resource* createResource(const Path& path) override {
			return AETHERION_NEW(m_allocator, PhysicsGeometry)(path, *this, m_system, m_allocator);
		}

		void destroyResource(Resource& resource) override {
			AETHERION_DELETE(m_allocator, static_cast<PhysicsGeometry*>(&resource));
		}
	...
	AETHERION_PLUGIN_ENTRY(physics) {
		PROFILE_FUNCTION();
		return AETHERION_NEW(engine.getAllocator(), PhysicsSystemImpl)(engine);
	}
