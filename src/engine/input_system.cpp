...
struct InputSystemImpl final : InputSystem {
	explicit InputSystemImpl(Engine& engine)
		...
		m_mouse_device = AETHERION_NEW(m_allocator, MouseDevice);
		m_mouse_device->type = Device::MOUSE;
		m_keyboard_device = AETHERION_NEW(m_allocator, KeyboardDevice);
		...
		if (m_gamepad_backend && m_gamepad_backend->init()) {
			...
			if (m_gamepad_backend) {
				AETHERION_DELETE(m_allocator, m_gamepad_backend);
	...
	~InputSystemImpl() {
		...
		for (Device* device : m_devices) {
			AETHERION_DELETE(m_allocator, device);
		}
	}
		...
		for (int i = 0; i < max_controllers; ++i) {
			...
			if (should_check) {
				...
				if (is_connected && !m_gamepad_devices[i]) {
					...
					GamepadDevice* device = AETHERION_NEW(m_allocator, GamepadDevice)(i);
