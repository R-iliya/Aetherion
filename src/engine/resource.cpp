...
void Resource::fileLoaded(Span<const u8> blob, bool success) {
	...
	if (startsWith(getPath(), ".aetherion/asset_tiles/")) {
		if (!load(blob)) ++m_failed_dep_count;
	}
	else if (blob.length() < sizeof(*header)) {
		logError("Invalid resource file, please delete .aetherion directory");
		++m_failed_dep_count;
	}
		...
		logError("Unsupported resource file version, please delete .aetherion directory");
		++m_failed_dep_count;
	}
...
void Resource::doLoad()
{
	...
	if (startsWith(m_path, ".aetherion/asset_tiles/")) {
		m_async_op = fs.getContent(m_path, cb);
	}
	else {	
		const FilePathHash hash = m_path.getHash();
		const Path res_path(".aetherion/resources/", hash, ".res");
