...
struct AETHERION_ENGINE_API SystemManager
{
...
struct AETHERION_ENGINE_API IModule
{
	virtual ~IModule() {}
...
struct AETHERION_ENGINE_API ISystem
{
	virtual ~ISystem();
	...
	#define AETHERION_PLUGIN_ENTRY(plugin_name) extern "C" AETHERION::ISystem* createPlugin_##plugin_name(AETHERION::Engine& engine)
#else
	#define AETHERION_PLUGIN_ENTRY(plugin_name) extern "C" AETHERION_LIBRARY_EXPORT Aetherion::ISystem* createPlugin(Aetherion::Engine& engine)
#endif
