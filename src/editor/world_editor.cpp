...
template <> AETHERION_EDITOR_API void writeToStream<const Path&>(OutputMemoryStream& stream, const Path& path);
	
template <> AETHERION_EDITOR_API Path readFromStream<Path>(InputMemoryStream& stream)
{
...
template <> AETHERION_EDITOR_API void writeToStream<Path>(OutputMemoryStream& stream, Path path) {
	StringView str = path;
	stream.write(str.begin, str.size());
	stream.write((char)0);
}

template <> AETHERION_EDITOR_API const char* readFromStream<const char*>(InputMemoryStream& stream)
{
	const char* c_str = (const char*)stream.getData() + stream.getPosition();
	stream.skip(stringLength(c_str) + 1);
	return c_str;
...
template <> AETHERION_EDITOR_API void writeToStream<const char*>(OutputMemoryStream& stream, const char* value)
{
		...
		bool execute() override
		{
			...
			for (int i = 0; i < m_entities.size(); ++i)
			{
				...
				for (ComponentType cmp_type : world->getComponents(m_entities[i])) {
					...
					Aetherion::save(cmp, m_old_values);
				}
		...
		void undo() override
		{
			...
			for (int i = 0; i < m_entities.size(); ++i)
			{
				...
				for (int j = 0; j < cmps_count; ++j)
				{
					...
					::Aetherion::load(new_component, blob);
				}
				PrefabHandle tpl;
				blob.read(tpl);
...
public:
	...
	void saveProject() {
		...
		if (!fs.saveContentSync(Path("aetherion.prj"), blob)) {
			logError("Failed to save aetherion.prj");
		}
	}

	void loadProject() override {
		OutputMemoryStream data(m_allocator);
		if (!m_engine.getFileSystem().getContentSync(Path("aetherion.prj"), data)) {
