...
static volatile bool once = [](){
	AETHERION_GLOBAL_FUNC(AssetBrowser::openEditor);
	return true;
}();
...
struct AssetBrowserImpl : AssetBrowser {
	...
	static TileState getState(const FileInfo& info, FileSystem& fs) {
		...
		const Path path(".aetherion/asset_tiles/", file_path_hash, ".lbc");
		...
		const Path compiled_path(".aetherion/resources/", file_path_hash, ".res");
	...
	void tileFile(FileInfo& tile, float size, bool selected) {
		...
		if (tile.tex) {
			...
			RenderInterface* ri = m_app.getRenderInterface();
			const Path path(".aetherion/asset_tiles/", tile.filepath.getHash().getHashValue(), ".lbc");
	...
	void deleteSelectedFiles() {
		...
		for (const Path& path : m_selected_resources) {
			const Path res_path(".aetherion/resources/", path.getHash().getHashValue(), ".res");
	...
	void recreateTiles() {
		for (FileInfo& fi : m_file_infos) {
			const Path path(".aetherion/asset_tiles/", fi.filepath.getHash().getHashValue(), ".res");
