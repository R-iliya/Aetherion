...
void init() {
	...
	for (const auto& m : map) {
		s_from_x11_keysym.insert(m.x11, m.aetherion);
		s_keycode_names[(u8)m.aetherion] = m.name;
	}
...
void logInfo() {
	struct utsname tmp;
	if (uname(&tmp) == 0) {
		Aetherion::logInfo("sysname: ", tmp.sysname);
		Aetherion::logInfo("nodename: ", tmp.nodename);
		Aetherion::logInfo("release: ", tmp.release);
		Aetherion::logInfo("version: ", tmp.version);
		Aetherion::logInfo("machine: ", tmp.machine);
	} else {
...
WindowHandle createWindow(const InitWindowArgs& args) {
	...
	XStoreName(display, win, args.name && args.name[0] ? args.name : "Aetherion App");
...
bool getNextFile(FileIterator* iterator, FileInfo* info) {
	...
	Aetherion::copyString(info->filename, dir_ent->d_name);
	return true;
}
...
u64 getLastModified(StringView _path) {
	...
	ret = tmp.st_mtim.tv_sec * 1000 + Aetherion::u64(tmp.st_mtim.tv_nsec / 1000000);
	return ret;
}
