...
static Aetherion::DefaultAllocator stack_node_allocator;
...
StackNode* StackTree::insertChildren(StackNode* root_node, void** instruction, void** stack) {
	...
	while (instruction >= stack) {
		StackNode* new_node = AETHERION_NEW(stack_node_allocator, StackNode)();
...
StackNode* StackTree::record() {
	...
	if (!m_root) {
		m_root = AETHERION_NEW(stack_node_allocator, StackNode)();
	...
	while (ptr >= stack) {
		...
		if (node->m_instruction != *ptr) {
			node->m_next = AETHERION_NEW(stack_node_allocator, StackNode);
