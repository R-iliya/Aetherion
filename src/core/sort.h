...
namespace Aetherion {
	...
	int partition(T* from,  T* to) {
		...
		if (*mid < *from) AETHERION_SWAP(*mid, *from);
		if (*(to - 1) < *from) AETHERION_SWAP(*(to - 1), *from);
		if (*(to - 1) < *mid) AETHERION_SWAP(*(to - 1), *mid);
		T* pivot = mid;
		T* i = from - 1;
