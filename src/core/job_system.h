...
AETHERION_CORE_API void waitAndTurnRed(Signal* signal);

AETHERION_CORE_API void wait(Counter* counter);

AETHERION_CORE_API void enter(Mutex* mutex);
AETHERION_CORE_API void exit(Mutex* mutex);

AETHERION_CORE_API bool init(u8 workers_count, IAllocator& allocator);
AETHERION_CORE_API IAllocator& getAllocator();
AETHERION_CORE_API void shutdown();
AETHERION_CORE_API u8 getWorkersCount();

// yield current job and push it to worker queue
AETHERION_CORE_API void moveJobToWorker(u8 worker_index);
// yield current job, push it to global queue
AETHERION_CORE_API void yield();

// run single job, increment on_finished counter, decrement it when job is done
AETHERION_CORE_API void run(void* data, void(*task)(void*), Counter* on_finish, u8 worker_index = ANY_WORKER);
// same as calling `run` `num_jobs` times, except it's faster
AETHERION_CORE_API void runN(void* data, void(*task)(void*), Counter* on_finish, u32 num_jobs);
...
void runLambda(F&& f, Counter* on_finish, u8 worker) {
	...
	if constexpr (sizeof(f) == sizeof(void*) && __is_trivially_copyable(F)) {
		...
		F* tmp = AETHERION_NEW(getAllocator(), F)(static_cast<F&&>(f));
		run(tmp, [](void* arg){
			F* f = (F*)arg;
			(*f)();
			AETHERION_DELETE(getAllocator(), f);
		}, on_finish, worker);
