...
#define AETHERION_NEW(allocator, ...) new (Aetherion::NewPlaceholder(), (allocator).allocate(sizeof(__VA_ARGS__), alignof(__VA_ARGS__))) __VA_ARGS__
#define AETHERION_DELETE(allocator, var) (allocator).deleteObject(var)

namespace Aetherion { struct NewPlaceholder {}; }
inline void* operator new(size_t, Aetherion::NewPlaceholder, void* where) { return where; }
inline void operator delete(void*, Aetherion::NewPlaceholder,  void*) { } 

namespace Aetherion {

struct AETHERION_CORE_API IAllocator {
	virtual ~IAllocator() {}
...
AETHERION_CORE_API IAllocator& getGlobalAllocator();
...
struct UniquePtr {
	...
	~UniquePtr() {
		if (m_ptr) {
			AETHERION_DELETE(*m_allocator, m_ptr);
		}
	}
	...
	template <typename... Args> static UniquePtr<T> create(IAllocator& allocator, Args&&... args) {
		return UniquePtr<T>(AETHERION_NEW(allocator, T) (static_cast<Args&&>(args)...), &allocator);
	}
