...
#define AETHERION_PROFILE_JOBS

namespace Aetherion::jobs {
...
AETHERION_FORCE_INLINE static void wake(WorkerTask& to_wake);
AETHERION_FORCE_INLINE static void wake(u32 num_jobs);
AETHERION_FORCE_INLINE static void wake();
AETHERION_FORCE_INLINE static void executeJob(const Job& job);
...
struct WorkStealingQueue {
	...
	AETHERION_FORCE_INLINE void pushAndWake(const Work& obj);
	// optimized batch push
	AETHERION_FORCE_INLINE void pushAndWakeN(const Work& obj, u32 num);

	AETHERION_FORCE_INLINE bool tryPop(Work& obj) {
		for (;;) {
	...
	AETHERION_FORCE_INLINE bool trySteal(Work& obj) {
		for (;;) {
			const i32 stealing_end = m_stealing_end;
...
struct WorkQueue {
	...
	AETHERION_FORCE_INLINE bool tryPop(Work& obj) {
		...
		Aetherion::MutexGuard guard(mutex);
		if (queue.empty()) {
			empty = 1;
			return false;
		}
	...
	AETHERION_FORCE_INLINE void pushAndWake(const Work& obj, WorkerTask* to_wake) {
		{
			Aetherion::MutexGuard guard(mutex);
			queue.push(obj);
			empty = 0;
...
struct System {
	...
	Aetherion::Mutex m_sleeping_sync;
...
AETHERION_FORCE_INLINE static FiberJobPair* popFreeFiber() {
...
AETHERION_FORCE_INLINE WaitingFiber* getWaitingFiberFromState(u64 state) {
	return (WaitingFiber*)((state & STATE_WAITING_FIBER_MASK) >> 16);
}

AETHERION_FORCE_INLINE u16 getCounterFromState(u64 state) {
	return u16(state & STATE_COUNTER_MASK);
}

AETHERION_FORCE_INLINE u64 makeStateValue(WaitingFiber* fiber, u16 counter) {
...
AETHERION_FORCE_INLINE static void scheduleFiber(FiberJobPair* fiber) {
...
AETHERION_FORCE_INLINE static bool trySteal(Work& work, WorkerTask* stealing_worker) {
...
AETHERION_FORCE_INLINE static bool tryPopWork(Work& work, WorkerTask* worker) {
...
AETHERION_FORCE_INLINE static bool popWork(Work& work, WorkerTask* worker) {
	while (!worker->m_finished) {
		...
		Aetherion::MutexGuard guard(g_system->m_sleeping_sync);
...
AETHERION_FORCE_INLINE static void switchFibers(i32 profiler_id) {
	WorkerTask* worker = getWorker();
...
AETHERION_FORCE_INLINE static void decCounter(Counter* counter) {
	for (;;) {
...
AETHERION_FORCE_INLINE static void addCounter(Counter* counter, u32 value) {
...
bool init(u8 workers_count, IAllocator& allocator) {
	...
	for (u32 i = 0; i < count; ++i) {
		WorkerTask* task = AETHERION_NEW(getAllocator(), WorkerTask)(*g_system, i);
	...
	for (u32 i = 0; i < count; ++i) {
		...
		if (task->create(StaticString<64>("Worker #", i), false)) {
			...
			AETHERION_DELETE(getAllocator(), task);
		}
	}
...
AETHERION_FORCE_INLINE static void wake(WorkerTask& worker) {
...
AETHERION_FORCE_INLINE static void wake(u32 num) {
