...
struct CullingSystemImpl final : CullingSystem
{
	...
	Sphere* addToCell(CellPage& cell, EntityPtr entity, const DVec3& pos, float radius)
	{
		...
		CellPage* new_cell = new (Aetherion::NewPlaceholder(), mem) CellPage;
	...
	AETHERION_FORCE_INLINE void doCulling(const CellPage& cell
		, const Frustum& frustum
		, CullResult* AETHERION_RESTRICT results
		, PagedList<CullResult>& list
		, u8 type)
	{
		PROFILE_FUNCTION();
		const Sphere* AETHERION_RESTRICT start = cell.spheres;
		const Sphere* AETHERION_RESTRICT end = cell.spheres + cell.header.count;
		const EntityPtr* AETHERION_RESTRICT sphere_to_entity_map = cell.entities;
