...
#if defined AETHERION_DEBUG && !defined __clang__
	#define USE_PIX
...
QueryHandle createQuery(QueryType type) {
	...
	switch(type) {
		case QueryType::STATS: {
			...
			Query* q = AETHERION_NEW(d3d->allocator, Query);
...
void destroy(ProgramHandle program) {
	...
	ASSERT(program);
	AETHERION_DELETE(d3d->allocator, program);
}
...
void destroy(TextureHandle texture) {
	...
	AETHERION_DELETE(d3d->allocator, texture);
}

void destroy(QueryHandle query) {
	checkThread();
	AETHERION_DELETE(d3d->allocator, query);
}
...
void shutdown() {
	d3d->shader_compiler.saveCache(".aetherion/shader_cache_dx");
...
bool init(void* hwnd, InitFlags flags) {
	...
	d3d->shader_compiler.loadCache(".aetherion/shader_cache_dx");
...
ProgramHandle allocProgramHandle() {
	Program* p = AETHERION_NEW(d3d->allocator, Program)(d3d->allocator);
	return p;
}

BufferHandle allocBufferHandle() {
	BufferHandle b = AETHERION_NEW(d3d->allocator, Buffer);
	b->heap_id = d3d->srv_heap.reserveID();
	return b;
}

TextureHandle allocTextureHandle() {
	TextureHandle t = AETHERION_NEW(d3d->allocator, Texture);
...
void destroy(BufferHandle buffer) {
	...
	AETHERION_DELETE(d3d->allocator, buffer);
}
