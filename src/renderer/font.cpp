...
bool FontManager::build()
{
	...
	if (m_atlas_texture) {
		...
		m_atlas_texture = AETHERION_NEW(m_allocator, Texture)(Path("draw2d_atlas"), texture_manager, m_renderer, m_allocator);
	}
...
Font* FontResource::addRef(int font_size)
{
	...
	Font* font = AETHERION_NEW(manager.m_allocator, Font)(manager.m_allocator);
	font->ref = 1;
	font->resource = this;
...
void FontResource::removeRef(Font& font)
{
	...
	if(font.ref == 0) {
		auto& manager = (FontManager&)m_resource_manager;
		AETHERION_DELETE(manager.m_allocator, &font);
...
{
	for (Font* font : m_fonts) {
		AETHERION_DELETE(m_allocator, font);
	}

	if (m_atlas_texture) {
		m_atlas_texture->destroy();
		AETHERION_DELETE(m_allocator, m_atlas_texture);
	}
}
...
{
	return AETHERION_NEW(m_allocator, FontResource)(path, *this, m_allocator);
}


void FontManager::destroyResource(Resource& resource)
{
	AETHERION_DELETE(m_allocator, static_cast<FontResource*>(&resource));
}
