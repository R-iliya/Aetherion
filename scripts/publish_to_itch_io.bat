...
cd PhysX\physx
call generate_projects.bat aetherion_vc17win64_static
if not %errorlevel%==0 pause
...
REM download dx
pushd ..\plugins
git.exe clone https://github.com/nem0/aetherionengine_dx.git dx
if not %errorlevel%==0 pause
popd
...
%msbuild_cmd% tmp/vs2022/aetherionEngine.sln /p:Configuration=RelWithDebInfo
if not %errorlevel%==0 pause
...
butler.exe push itch_io mikulasflorek/aetherion-engine:win-64-gl
if not %errorlevel%==0 pause
...
git clean -f -x -d ..\data
xcopy /E /Y ..\data itch_io
butler.exe push itch_io mikulasflorek/aetherion-engine:win-64-dx
pause
