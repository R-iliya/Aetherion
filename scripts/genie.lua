...
if plugin "physics" then
	...
	defines { "BUILDING_PHYSICS", "AETHERION_STATIC_PHYSX" }
...
if plugin "renderer" then
	...
	if use_basisu then
		defines { "AETHERION_BASIS_UNIVERSAL" }
...
if build_app then
	exe_project "app"
		...
		if #plugins > 0 and dynamic_plugins then
			...
			defines { "AETHERIONENGINE_PLUGINS=" .. def }
		end
...
if build_luau then
	...
	if not luau_dynamic then
		solution "AetherionEngine"
			configuration { "vs20*" }
			defines { "AETHERION_LUAU_ANALYSIS" }
			configuration {}
	end
else	
...
io.write "#ifdef AETHERION_PLUGIN_DECLS\n"
	for _, plugin in ipairs(plugin_creators) do
		io.write([[extern "C" ISystem* createPlugin_]] .. plugin .. "(Engine&);\n")
	end
io.write "#elif defined AETHERION_EDITOR_PLUGINS_DECLS\n"
	if not dynamic_plugins then
		for _, plugin in ipairs(plugin_creators) do
			io.write([[extern "C" Aetherion::StudioApp::IPlugin* setStudioApp_]] .. plugin .. "(StudioApp&);\n")
		end
	end
io.write "#elif defined AETHERION_EDITOR_PLUGINS\n"
	if not dynamic_plugins then
		for _, plugin in ipairs(plugin_creators) do
			io.write "{\n"
...
io.write "#elif defined AETHERION_PLUGINS_STRINGS\n"
	if dynamic_plugins then
		for _, plugin in ipairs(plugin_creators) do
			io.write("\"" .. plugin .. "\", ")
		end
